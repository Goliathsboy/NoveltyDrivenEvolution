...
import intersectionmanagement.training.ControllerNoveltyScore;
import intersectionmanagement.training.PopulationInformation;
...
public class BasicEA implements EvolutionaryAlgorithm, MultiThreadable,
	...
	public void iteration() {
		...
		// Start addition
		if (this.actualThreadCount != 0) {
			calculatePopulationScores(); // Calculates the score for all the population's members
		}
		// End addition
	...
	private void preIteration() {
		...
		PopulationInformation.cyclicBarrier = new CyclicBarrier(populationSize);
		PopulationInformation.currentPopulationValues = new ArrayList<>(populationSize);
		PopulationInformation.populationSize = populationSize;
		//End addition
		...
		pscore.setThreadCount(populationSize);
		pscore.process();
